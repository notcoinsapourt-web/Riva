...
async def site_hub(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    await state.clear()
    pending_orders = int(await session.scalar(select(func.count()).select_from(WebOrder).where(WebOrder.status.in_(["pending", "approved", "processing"]))) or 0)
    pending_deposits = int(await session.scalar(select(func.count()).select_from(WebDeposit).where(WebDeposit.status == "pending")) or 0)
    ...
    await edit_or_send(
        ...
        reply_markup=keyboard(
            [button("📦 سفارش‌های سایت", callback_data=AdminCallback(section="web", action="orders").pack(), style="primary")],
            [button("💳 شارژهای سایت", callback_data=AdminCallback(section="web", action="deposits").pack())],
            [button("👥 کاربران سایت", callback_data=AdminCallback(section="web", action="users").pack())],
    ...
    for order, email in orders:
        ...
        rows.append([
            button(
                ...
                callback_data=AdminCallback(section="web", action="order", entity_id=int(order.id)).pack(),
            )
        ])
    rows.append([button("↩️ بخش سایت", callback_data=AdminCallback(section="web", action="show").pack())])
    await edit_or_send(
        callback,
...
async def show_order(event: Message | CallbackQuery, order_id: int, session: AsyncSession) -> None:
    ...
    for status in transitions:
        rows.append([
            button(
                labels[status],
                callback_data=AdminCallback(section="web", action=f"os_{status}", entity_id=int(order.id)).pack(),
                style="danger" if status == "cancelled" else "success",
            )
        ])
    rows.extend([
        [button("📝 یادداشت مدیریت", callback_data=AdminCallback(section="web", action="note", entity_id=int(order.id)).pack())],
        [button("↩️ سفارش‌های سایت", callback_data=AdminCallback(section="web", action="orders").pack())],
    ])
    note = f"\n\n<b>یادداشت مدیریت</b>\n{h(order.admin_note)}" if order.admin_note else ""
...
async def web_order_detail(callback: CallbackQuery, callback_data: AdminCallback, session: AsyncSession) -> None:
...
async def web_order_status(callback: CallbackQuery, callback_data: AdminCallback, session: AsyncSession) -> None:
    new_status = callback_data.action.removeprefix("os_")
    now = datetime.now(timezone.utc)
    order = await session.scalar(select(WebOrder).where(WebOrder.id == callback_data.entity_id).with_for_update())
    if not order:
    ...
    if new_status == "cancelled" and order.refunded_at is None:
        wallet = await session.scalar(select(WebWallet).where(WebWallet.user_id == order.user_id).with_for_update())
        if wallet is None:
            await callback.answer("کیف پول کاربر پیدا نشد.", show_alert=True)
            return
        before = int(wallet.balance)
        after = before + int(order.total_amount)
        wallet.balance = after
        wallet.updated_at = now
        session.add(WebWalletTransaction(
            wallet_id=wallet.id,
...
async def ask_web_order_note(callback: CallbackQuery, callback_data: AdminCallback, session: AsyncSession, state: FSMContext) -> None:
    ...
    await edit_or_send(
        ...
        reply_markup=keyboard([button("لغو", callback_data=AdminCallback(section="web", action="order", entity_id=int(order.id)).pack(), style="danger")]),
    )


@router.message(WebOrderNoteState.text)
async def save_web_order_note(message: Message, session: AsyncSession, state: FSMContext) -> None:
    ...
    for deposit, email in items:
        ...
        rows.append([
            button(
                ...
                callback_data=AdminCallback(section="web", action="deposit", entity_id=int(deposit.id)).pack(),
            )
        ])
    rows.append([button("↩️ بخش سایت", callback_data=AdminCallback(section="web", action="show").pack())])
    await edit_or_send(callback, "<b>💳 شارژهای سایت</b>\n\n" + ("آخرین درخواست‌ها:" if items else "هنوز درخواست شارژی ثبت نشده است."), reply_markup=keyboard(*rows))


async def show_deposit(event: Message | CallbackQuery, deposit_id: int, session: AsyncSession) -> None:
    row = (await session.execute(
        select(WebDeposit, WebUser.email)
        .join(WebUser, WebUser.id == WebDeposit.user_id)
    ...
    if not row:
        ...
        return
    deposit, email = row
    rows = [[button("🧾 مشاهده رسید", callback_data=AdminCallback(section="web", action="proof", entity_id=int(deposit.id)).pack())]]
    if deposit.status == "pending":
        rows.extend([
            [button("✅ تأیید و شارژ کیف پول", callback_data=AdminCallback(section="web", action="da", entity_id=int(deposit.id)).pack(), style="success")],
            [button("❌ رد درخواست", callback_data=AdminCallback(section="web", action="dr", entity_id=int(deposit.id)).pack(), style="danger")],
        ])
    rows.append([button("↩️ شارژهای سایت", callback_data=AdminCallback(section="web", action="deposits").pack())])
    tx_hash = f"\nهش تراکنش: <code>{h(deposit.transaction_hash)}</code>" if deposit.transaction_hash else ""
    await edit_or_send(
        event,
...
async def web_deposit_detail(callback: CallbackQuery, callback_data: AdminCallback, session: AsyncSession) -> None:
...
async def web_deposit_proof(callback: CallbackQuery, callback_data: AdminCallback, session: AsyncSession) -> None:
    ...
    await callback.message.answer_document(
        BufferedInputFile(deposit.proof_bytes, filename=deposit.proof_name or f"{deposit.number}.jpg"),
...
async def review_web_deposit(callback: CallbackQuery, callback_data: AdminCallback, session: AsyncSession) -> None:
    deposit = await session.scalar(select(WebDeposit).where(WebDeposit.id == callback_data.entity_id).with_for_update())
    if not deposit:
    ...
    if callback_data.action == "da":
        wallet = await session.scalar(select(WebWallet).where(WebWallet.user_id == deposit.user_id).with_for_update())
        if wallet is None:
            await callback.answer("کیف پول کاربر پیدا نشد.", show_alert=True)
            return
        before = int(wallet.balance)
    ...
    await session.flush()
    await callback.answer("درخواست شارژ بررسی شد.")
...
async def web_users(callback: CallbackQuery, session: AsyncSession) -> None:
    result = await session.execute(
        ...
        .join(WebWallet, WebWallet.user_id == WebUser.id)
        .order_by(WebUser.created_at.desc())
        .limit(30)
    )
    items = result.all()
    text_rows = [
        f"• <code>{h(user.email)}</code> — <b>{money(balance)}</b>" + (f" — {h(user.phone)}" if user.phone else "")
        for user, balance in items
    ]
    await edit_or_send(
        callback,
        "<b>👥 کاربران سایت</b>\n\n" + ("\n".join(text_rows) if text_rows else "هنوز کاربری در سایت ثبت‌نام نکرده است."),
        reply_markup=keyboard([button("↩️ بخش سایت", callback_data=AdminCallback(section="web", action="show").pack())]),
    )
